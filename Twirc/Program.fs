...
let main args =
    ...
    | channels ->
        ...
        | Some x ->
            match x.username, x.password with
            | "",_ ->
                printError "Username not provided for auto-login"
            | _,"" ->
                printError "Password not provided for auto-login"
            | _ ->
                let jc = channels |> List.append x.joinChannels
                run x.username x.password jc
        | _ ->
